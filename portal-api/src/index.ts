...
app.get('/api/projects/:projectId/nfts', async (req: any, res: any) => {
  try {
...
app.get('/api/projects/:projectId/discord', async (req: any, res: any) => {
  try {
    const { projectId } = req.params;
...
app.get('/api/projects/wallet/:wallet', async (req: any, res: any) => {
  try {
...
app.get('/api/chat/sessions/project/:projectId', async (req: any, res: any) => {
  try {
    const { projectId } = req.params;
...
app.get('/api/chat/messages/session/:sessionId', async (req: any, res: any) => {
  try {
...
app.delete('/api/projects/privy/:privyId', async (req: any, res: any) => {
  try {
    const { privyId } = req.params;
...
// Handle Discord bot webhook events
app.post('/api/discord/bot-events', async (req: any, res: any) => {
  try {
    const { event, guildId, memberCount: requestMemberCount } = req.body;

    if (!guildId) {
      console.log('[Webhook] Missing guildId in webhook event');
    ...
    if (!discordRecord) {
      console.log(`[Webhook] No Discord record found for guildId: ${guildId}`);
    ...
    if (event === 'guildCreate') {
      // Extract memberCount from the request or default to what we have in the database
      const memberCount = requestMemberCount || discordRecord.memberCount || 0;

      // Update Discord record to mark bot as added
      await prisma.discord.update({
...
**Great news!** The verification bot has been successfully added to your Discord server "${project.projectName || 'Your Discord Server'}".
...
${
  memberCount >= 4
    ? `## Congratulations! 🚀\n\nYou have **${memberCount} members** in your server, which meets the requirement for Level 3!\n\nI'll process your level-up right away.`
...
Continue growing your community and sharing valuable scientific content to progress to Level 4!`,
      ...
      console.log(`[Webhook] Successfully registered bot for Discord server ID: ${guildId}`);
      return res.json({
        success: true,
        message: 'Bot installation recorded successfully',
      });
    } else if (event === 'stats_update') {
      // Handle stats update event
      const memberCount = requestMemberCount || discordRecord.memberCount;
      const messagesCount = req.body.messagesCount || 0;
      const papersCount = req.body.papersCount || 0;
      ...
      await prisma.discord.update({
        ...
        data: {
          memberCount:
            memberCount > discordRecord.memberCount ? memberCount : discordRecord.memberCount,
          messagesCount:
            messagesCount > discordRecord.messagesCount
              ? messagesCount
              : discordRecord.messagesCount,
          papersShared:
            papersCount > discordRecord.papersShared ? papersCount : discordRecord.papersShared,
          updatedAt: new Date(),
        },
      });
    ...
    console.error('[Webhook] Error handling Discord bot event:', error);
    return res.status(500).json({
      success: false,
...
function startPeriodicLevelCheck() {
  // Check user progress every 4 hours
  setInterval(
    async () => {
      console.log('Running periodic level check for all users...');
      try {
        // Get all users
        ...
        for (const project of projects) {
          await checkUserProgressAndSendGuidance(project);
        }
      } catch (error) {
  ...
  // Check for users at level 2 and 3 more frequently
  setInterval(
    async () => {
      console.log('Running focused check for users at level 2 and 3...');
      try {
        // Get users at level 2 and 3
        const projects = await prisma.project.findMany({
          where: {
            level: {
              in: [2, 3],
        ...
        console.error('Error in focused level check:', error);
      }
    },
    15 * 60 * 1000
  ); // Every 15 minutes

  // Check specifically for level 3 users close to level 4 very frequently
  setInterval(
    async () => {
      console.log('Running targeted check for level 3 users close to level 4...');
      try {
        // Get only level 3 users with Discord data
        const projects = await prisma.project.findMany({
          where: {
            level: 3,
            Discord: {
        ...
        // Check each level 3 project that might be close to level 4
        for (const project of projects) {
          if (!project.Discord) continue;

          // Calculate how close they are to meeting requirements
          const membersPercent = Math.min(
            100,
            Math.round((project.Discord.memberCount / 10) * 100)
          );
          const papersPercent = Math.min(
            100,
            Math.round((project.Discord.papersShared / 25) * 100)
          );
          const messagesPercent = Math.min(
            100,
            Math.round((project.Discord.messagesCount / 100) * 100)
          );

          // If they're at least 80% of the way there for all metrics, check more frequently
          const isCloseToLevelUp =
            membersPercent >= 80 && papersPercent >= 80 && messagesPercent >= 80;

          if (isCloseToLevelUp) {
            console.log(`Project ${project.id} is close to level 4 - checking for level-up...`);
            await checkDiscordLevelProgress(project);

            // If at least one metric is exactly at threshold or higher, update real-time
            if (
              project.Discord.memberCount >= 10 ||
              project.Discord.papersShared >= 25 ||
              project.Discord.messagesCount >= 100
            ) {
              // Instead of trying to force an update, just check again with current data
              console.log(
                `Project ${project.id} has met at least one Level 4 requirement threshold`
              );

              // Get a fresh copy of the project with the latest Discord data
              ...
              // Check level up with refreshed data
              if (refreshedProject) {
        ...
        console.error('Error in targeted level 3 check:', error);
      }
    },
    5 * 60 * 1000
  ); // Every 5 minutes
...
app.post('/discord/stats-update', async (req, res) => {
  try {
    ...
    console.log(`[API] Received direct stats update for guild ${guildId}:`, req.body);

    // Verify API key
    if (apiKey !== API_KEY) {
      console.warn(`[API] Unauthorized stats update attempt with invalid API key`);
      return res.status(401).json({ success: false, error: 'Unauthorized' });
    }
    ...
    if (discord) {
      // Always keep the highest message count (never decrease)
      const newMessagesCount =
        messagesCount !== undefined
          ? Math.max(messagesCount, discord.messagesCount)
          : discord.messagesCount;

      // Same for papers
      const newPapersShared =
        papersShared !== undefined
          ? Math.max(papersShared, discord.papersShared)
          : discord.papersShared;

      console.log(`[API] Updating Discord stats for guild ${guildId}:
                Messages: ${discord.messagesCount} → ${newMessagesCount}
                Papers: ${discord.papersShared} → ${newPapersShared}`);
      ...
      const updatedRecord = await prisma.discord.update({
        ...
        data: {
          ...
          papersShared: newPapersShared,
          qualityScore: qualityScore !== undefined ? qualityScore : discord.qualityScore,
          updatedAt: new Date(),
        },
      });

      console.log(`[API] Discord stats updated successfully for guild ${guildId}`);
      ...
      console.warn(`[API] No Discord record found for server ID: ${guildId}`);
      return res.status(404).json({
        success: false,
        error: 'No Discord server record found with this ID',
      });
...
async function checkDiscordLevelProgress(project: Project) {
  const discord = await prisma.discord.findFirst({
  ...
  // Level 3 requirements: Discord created, 4+ members
  if (currentLevel === 2 && discord.botAdded && discord.memberCount >= 4) {
    shouldUpdateLevel = true;
    newLevel = 3;
  }

  // Level 4 requirements: 10+ members, 25+ papers, 100+ messages
  // Note: Removed quality score requirement to match checkAndPerformLevelUp
  if (
    currentLevel === 3 &&
  ...
  if (shouldUpdateLevel) {
    ...
    console.log(`Project ${project.id} automatically progressed to level ${newLevel}`);
    ...
    if (userId && activeConnections[userId]) {
      ...
      // Create appropriate level-up message based on the new level
      let levelUpMessage = '';
...
**Congratulations!** Your BioDAO community now has **${discord.memberCount} members**, which means you've advanced to **Level 3!**`;

        nextRequirementsMessage = `## Level 3 Requirements:

To advance to Level 4, you'll need to:

1. **Grow your community to 10+ members** (currently: ${discord.memberCount})
2. **Share 25+ scientific papers** in your Discord (currently: ${discord.papersShared || 0})
3. **Reach 100+ quality messages** (currently: ${discord.messagesCount || 0})
...
- **${discord.papersShared} scientific papers shared**
- **${discord.messagesCount} messages** in your server
...
Is there anything specific you'd like to prepare for the team call?`;
      ...
      // Send complete level-up message through WebSocket
      ws.send(
        JSON.stringify({
          type: 'level_up',
          level: newLevel,
      ...
      // After a short delay, send the next steps message
      setTimeout(async () => {
        // Save the next steps message to chat
        await saveChatMessage(sessionId, nextRequirementsMessage, true, 'GUIDANCE', true);

        // Send the next steps message to WebSocket
        ws.send(
          JSON.stringify({
            type: 'message',
        ...
        // If level 4 was reached, send the sandbox email
        if (newLevel === 4 && updatedProject.email) {
          await sendLevelUpEmail(updatedProject.email, newLevel);
          await sendSandboxEmail(updatedProject);
        }
      }, 2000);
    }
  }
...
// Add a new API endpoint to get Discord server information
app.get('/api/discord/info/:serverId', async (req: any, res: any) => {
  try {
...
app.post('/api/projects/privy/:privyId', async (req: any, res: any) => {
  try {
    const { privyId } = req.params;
    const {
      fullName,
  ...
  try {
    ...
    if (levelUp) {
      ...
      if (userId && activeConnections[userId]) {
        ...
        if (refreshedProject) {
          await checkAndPerformLevelUp(refreshedProject, ws);
        }
      } else {
