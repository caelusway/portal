...
export function CoreAgent() {
  ...
  // Add an enhanced version of the checkProgress function to do a more thorough check when close to level requirements
  // Insert after the existing checkProgress function

  // Enhanced progress check that focuses on level-up requirements
  const checkLevelUpRequirements = useCallback(async () => {
    if (!projectId || !levelProgress || !userLevel) return;

    console.log('[CoreAgent] Performing enhanced level-up requirement check');

    try {
      // Force a direct API level check instead of relying on WebSocket
      const currentApiLevel = await fetchCurrentLevel(projectId);

      // If API shows a different level than what we have locally, update immediately
      if (currentApiLevel !== null && currentApiLevel !== userLevel) {
        console.log(
          `[CoreAgent] API level check detected level change: ${userLevel} → ${currentApiLevel}`
        );
        setUserLevel(currentApiLevel);

        // Show toast to notify user of level change
        toast({
          title: '🚀 Level Up!',
          description: `You've advanced to Level ${currentApiLevel}!`,
          variant: 'default',
          duration: 5000,
        });
      ...
      // Level-specific forced checks
      if (userLevel === 1) {
        // For level 1, check NFTs status directly
        fetchNFTs();
      } else if (userLevel === 2 || userLevel === 3) {
        // For levels 2-3, force a Discord stats refresh
        getDiscordStats();

        // Now check progress directly with the server, forcing a level check
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
          wsRef.current.send(
            JSON.stringify({
              type: 'check_progress',
          ...
          console.log('[CoreAgent] Sent forced progress check to server');
        }
      }

      // Set last level check timestamp
      setLastLevelCheck(Date.now());
    } catch (error) {
      console.error('[CoreAgent] Error in level-up requirements check:', error);
    }
  }, [
    projectId,
    userLevel,
  ...
  // Modify the useEffect that watches levelProgress to use the enhanced check
  useEffect(() => {
    if (!levelProgress || !projectId) return;

    // Create a progress tracker for each level requirement
    const progressValues = Object.values(levelProgress.progress || {});

    // Check if all requirements are met or close to being met
    const allMet = progressValues.every((p) => p.percent >= 99);
    const anyClose = progressValues.some((p) => p.percent >= 90 && p.percent < 100);
    const mostlyMet =
      progressValues.filter((p) => p.percent >= 80).length >= progressValues.length * 0.75;

    // If all requirements are met, check level immediately
    if (allMet && Date.now() - lastLevelCheck > 10000) {
      console.log('[CoreAgent] All requirements appear to be met, checking for level update');
      checkLevelUpRequirements();
    }
    // If any are close to complete or most requirements are mostly met, check more often
    else if ((anyClose || mostlyMet) && Date.now() - lastLevelCheck > 15000) {
      console.log('[CoreAgent] Some requirements are close to completion, checking progress');
      checkLevelUpRequirements();
    }
  }, [levelProgress, projectId, userLevel, lastLevelCheck, checkLevelUpRequirements]);
