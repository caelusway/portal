...
const POIUploader = ({ apiKey }) => {
  ...
  return (
    <div className="poi-uploader">
      ...
      <button onClick={submitPOI} disabled={loading || files.length === 0}>
      ...
      {result && (
        <div className="result">
          ...
          <div className="result-details">
            <p>
              <strong>Merkle Root:</strong> {result.result.root}
            </p>
            <p>
              <strong>Contract Address:</strong> {result.result.transaction.recipient}
            </p>
            <p>
              <strong>Files Processed:</strong> {result.result.files.length}
            </p>
            ...
            <pre>{JSON.stringify(result.result.transaction, null, 2)}</pre>
          </div>
        </div>
      )}
    </div>
...
<template>
  <div class="poi-uploader">
    ...
    <button @click="submitPOI" :disabled="loading || files.length === 0">
      {{ loading ? 'Generating Proof...' : 'Generate Proof of Invention' }}
    </button>
...
async function handlePOISubmission(files, apiKey) {
  ...
  if (totalSize > maxSize) {
    throw new Error(
      `Total file size (${Math.round(totalSize / 1024 / 1024)}MB) exceeds 100MB limit`
    );
  }
